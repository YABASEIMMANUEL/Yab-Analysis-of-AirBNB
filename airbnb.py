...
if selected == "Home":
    ...
    #st.image('C:\\Users\\LENOVO\\Desktop\\Guvi Proj\\Airbnb Analysis\\Airbnb Proj\\aribnb.gif')
# Read the image file and encode it to base64
    with open('C:\\Users\\LENOVO\\Desktop\\Guvi Proj\\Airbnb Analysis\\Airbnb Proj\\aribnb.gif', 'rb') as f:
        ...
        if not st.session_state['df'].empty:
            ...
            with col1:
                ...
                fig.update_traces(marker=dict(line=dict(width=0.5, color='black')),
                                  ...
                                  hovertemplate='<b>%{y}</b><br>Listings: %{x}')
            ...
            with col2:
                ...
                fig.update_traces(textposition='outside', textinfo='value+label', pull=[0.1, 0, 0, 0],
                                  hovertemplate='<b>%{label}</b><br>Counts: %{value}')
                ...
                fig = px.choropleth(country_df,
                                    ...
                                    hover_name='country',
                                    hover_data={'Total_Listings': True},
...
if selected == "Explore" and not st.session_state['df'].empty:
    ...
    with col1:
        ...
        fig.update_traces(textposition='outside',
                          hovertemplate='<b>%{x}</b><br>Avg price: %{y}')
        ...
        # Availability by Property_type Bar Chart
        pra_df = df.query(query).groupby('property_type', as_index=False)['availability_365'].mean().sort_values(by='availability_365')
        fig = px.bar(data_frame=pra_df,
                     x='property_type',
                     y='availability_365',
                     color='availability_365',
                     title='Avg Availability by Property Type',
                     color_continuous_scale=px.colors.sequential.Rainbow,
                     text='availability_365')
        fig.update_traces(textposition='outside',
                          hovertemplate='<b>%{x}</b><br>Avg Availability: %{y}')
    ...
    with col2:
        ...
        fig.update_traces(marker=dict(line=dict(width=0.5, color='black')),
                          hovertemplate='<b>%{location}</b><br>Avg price: %{marker.size}')
        fig.update_layout(title_font_size=20, title_font_family="Arial")
        st.plotly_chart(fig, use_container_width=True)
        ...
        fig = px.scatter_geo(data_frame=country_df,
                             ...
                             color_continuous_scale='agsunset')
        fig.update_traces(marker=dict(line=dict(width=0.5, color='black')),
                          hovertemplate='<b>%{location}</b><br>Avg Availability: %{marker.size}')
        fig.update_layout(title_font_size=20, title_font_family="Arial")
        st.plotly_chart(fig, use_container_width=True)
        ...
        # Categorize ratings into 5 groups
        ...
        # Grouping by rating and counting reviews
